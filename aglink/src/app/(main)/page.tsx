...
import { SearchIcon, BookmarkIcon, BookmarkItem } from "@/components/domain/home";
    ...
    <div className="bg-gray-50 min-h-screen font-sans p-4 sm:p-6 lg:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="flex justify-between items-center mb-6">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">ようこそ、usernameさん！</h1>
          <button
        ...
        <main className="space-y-6">
          ...
          <section className="bg-green-100/60 rounded-2xl p-4 flex justify-around items-center">
            {homeIcons.map((icon, index) => (
              <img key={index} src={icon.src} alt={icon.alt} className="h-14 w-14 sm:h-16 sm:w-16 opacity-70" />
            ))}
          </section>
          ...
          <section className="bg-[#8CB389] w-full p-4 sm:p-6 rounded-2xl shadow-md flex flex-col">
            <div className="flex justify-between items-center mb-4 flex-shrink-0">
               <div className="flex items-center space-x-3">
                 <BookmarkIcon />
                 <h2 className="text-white font-bold text-2xl sm:text-3xl">ブックマーク</h2>
               </div>
               <a href="#" className="text-white/90 hover:text-white transition-colors text-sm sm:text-base">ブックマーク一覧へ</a>
            </div>
